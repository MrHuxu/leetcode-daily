\>\> [题目链接](https://leetcode.com/explore/featured/card/june-leetcoding-challenge-2021/604/week-2-june-8th-june-14th/3774/)

题意: 实现一个日历, 可以在日历上加入会议时间, 如果会议时间不与其他位置冲突, 则返回 true, 否则返回 false

解答: 声明一个二维数组 `data [][]int` 使用 data[i][0] 表示一个会议的开始, data[i][1] 表示一个会议的结束, 整体会议时间从小到大排列, 然后新插入会议时, 先根据 data 判断时间合不合法, 不合法则返回 false, 合法则更新 data 并返回 true
